...
test('Episodes renders the list of missions', () => {
    const { queryAllByTestId, getByText, rerender } = render(<Episodes episodes={[]} />)

    expect(queryAllByTestId(/episode-list/i)).toHaveLength(0)

    rerender(<Episodes episodes={mockEpisodesData} />)

    expect(getByText(/vanishing of/i)).toBeInTheDocument()
