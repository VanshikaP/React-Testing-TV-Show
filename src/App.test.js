...
test('clicking on the button fetches data and renders it to the DOM', async () => {
   ...
   const { getByText, queryAllByTestId } = await render(<App />)

    // query for the button & click on the button & query for the missions array / assert that it is rendered
    // fireEvent.click(getByText(/get data/i))

    // getByText(/select/i)
    userEvent.click(getByText(/select/i))
    ...
    // can do other assertions out here. Await means this code will not run until the 
